...
"""
    ...
    def royxat_qoshish(self,id:int=None,number:int=None,nick_name:str=None):
        # SQL_EXAMPLE ="INSERT INTO myfiles_teacher(id,name,email)VALUES(1,"john","john@hgmail.com")"

        sql="""
        INSERT INTO royxat ( id,number, nick_name ) VALUES(?, ?, ?)
        """
        self.execute(sql, parameters=(id,number,nick_name), commit=True)
...
"""
    ...
    def select_random_zakaz(self):
        sql = """
        SELECT * FROM zakaz ORDER BY random() LIMIT 10
        """
        return self.execute(sql, fetchall=True)
